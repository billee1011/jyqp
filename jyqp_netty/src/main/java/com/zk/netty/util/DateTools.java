...
public class DateTools {
	...
	private static void initEndDateListMap(){
		...
		for(int day=1; day<8; day++){
			...
			String currDate = DateTools.nDaysAftertoday(day-1);
			endDateMap.put("date", currDate);
			endDateMap.put("desc", DateTools.getWeekdayString(currDate));
	...
	/**
	 * 格式化 为 yyyy年MM月dd日  HH时mm分ss秒
	 * @param date
	 * @return
	 */
	public static String formatChinesesDateTime(Date date){
		SimpleDateFormat df = new SimpleDateFormat("yyyy年MM月dd日HH时mm分ss秒");
		return df.format(date);
	}
	
   /**
	 * 格式化 为 yyyyMMddHHmmss
	 * @param date
	 * @return
	 */
